...
| 09-24 | Week 4: ASan reports a leak he wrote; after the fix it reports nothing, including on the throwing path (RAII class, then `unique_ptr`, then read the copy error) | code-new | 1h ("no idea an hour?") | — | 38 min (17:26–18:04) | 0.63x | Started ~17:30 incl. new `cpp_memory/`-style dir + CMakeLists. Step 1 (ctor/dtor + leak) skipped — done 09-22. |
| 09-24 | Move semantics, core: `std::move` a `unique_ptr`, observe the moved-from state, then a noisy class showing copy vs move ctor (predict which runs) | code-new | — | 30–40 min (asked for mine; he said he was getting antsy) | 51 min (17:03–17:54, 09-25) | ~1.5x | Not started 09-24: he stopped at 18:08. Ran 09-25 in a noisy location (his words: hard to focus). Scope grew: vector reallocation + `noexcept` + missing-move-ctor fallback were added on top of the plan, and "what does `std::move` do" took four guesses before I gave the answer. |
| 09-25 | `shared_ptr`: predict `use_count` through copy/scope/move/reset, whether the pointee is copied, and when the destructor runs | code-new | — | 20–25 min | ~25 min (17:55 to ~18:20; end not timestamped) | ~1.1x | Given 17:55, after he chose to keep going. Guided, predict-then-run. Instructions got buried under my tool calls and had to be restated (~2 min lost). |
| 09-26 | Syllabus weeks 8–9, Components I + II: Bench Rig deliverable done (real I2C IMU on the week-7 sensor bus with the core untouched; PID motor with plotted step response; four-questions README) | read + code-new | — | 20–25 h (two syllabus weeks) | open | — | Given while scoping Seth's critique, to size what Part 11 had to give up. A prior, not a measurement — nothing like this has been timed yet. |
| 09-26 | Use-after-free under ASan (predicted report) + same bug under valgrind; week 4 evidence-complete | code-new | — | 25–30 min | open | — | Exercise given 09-25 18:20, not started. Guided. |
